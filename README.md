...
### Арт-стиль
Яркий, мультяшный, дружелюбный. Главные звезды — зверьки: они должны быть максимально круглыми, "желеобразными" и выразительными. Анимации преувеличенные, с большим количеством тряски и деформаций.

### Юмор
Акцент на визуальном юморе. Нелепые столкновения, комичные полеты с арены и забавные анимации — основа привлекательности игры.
...
### Управление кэшем и версиями

Для обеспечения того, чтобы пользователи всегда получали последнюю версию игры, реализованы следующие механизмы:

**Cache Busting:**
- HTTP заголовки `Cache-Control: no-cache, no-store, must-revalidate`
- Версионные параметры в URL ресурсов (`?v=1.0.0`)
- Автоматическое добавление timestamp к сборочным файлам через Vite

**Проверка версии:**
- Отображение текущей версии в правом верхнем углу игры
- Логирование версии в консоль браузера при загрузке
- Соответствие версий между `package.json`, кодом и HTML

**Для пользователей:**
- Если игра работает некорректно, попробуйте принудительный refresh (Ctrl+F5 или Cmd+Shift+R)
- Версия отображается в правом верхнем углу интерфейса
- В консоли браузера (F12) можно увидеть время загрузки и версию игры